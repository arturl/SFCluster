...
namespace ImageRecognizerLib
{
    ...
    class CNTKImageRecognizer
    {
        ...
        std::wstring classifyImage(const uint8_t* image_data, size_t image_data_len, int loggingLevel);

    public:
        CNTKImageRecognizer(const std::wstring & modelFile, const std::wstring & classesFile);
        ~CNTKImageRecognizer();
        std::wstring RecognizeObject(const uint8_t* bytes, size_t size, int loggingLevel);
